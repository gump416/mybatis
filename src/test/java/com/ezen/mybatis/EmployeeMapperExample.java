...
public class EmployeeMapperExample {
	...
	public static void main(String[] args) {
		...
		// Not Auto Commit
		//SqlSession sqlSession = sqlSessionFactory.openSession(false); //복잡한경우에  트랜잭션 관리할때는 false
		//sqlSession.commit();
		//sqlSession.rollback();
...
//		List<Employee> employeeList = sqlSession.selectList(namespace + ".findAll");
		EmployeeMapper employeeMapper = sqlSession.getMapper(EmployeeMapper.class); //내가지정한인터페이스타입만지정해주면 Mybatis가 알아서 구현체만들어서 return해줌 받을땐 지정인터페이스로..
		System.out.println(employeeMapper);
		List<Employee> employeeList= employeeMapper.findAll(); //Dao안쓰고 Mapper권장,구현체 만들필요없어서 좋음
